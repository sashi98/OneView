...
printUsage() {
    echo "NAME"
    echo "    $0 - Sets up you generic DB on your local VM. Imports a new schema with the name you provided and password as Schmedge1234"
    echo ""
    echo "SYNOPSIS"
    echo "    $0  [Payor Major Version For the DB] [Type of Schema db - Give db for oltp and dw for DW] [Desired Schema name. DB Type will be appended to this name] "
    echo ""
    echo "EXAMPLE"
    echo "    $0 19.6 db PAYOR_19_6"
    echo ""
}
...
function checkVersionValid {
    ...
    then
        echo "Will be importing DB for $VERSION_NUMBER Version"

    else
        echo "$VERSION_NUMBER is not a valid Version. Only Major Versions are allowed."
        exit 1
    fi
}
...
case $DB_TYPE in
    db|dw) echo "" ;;
    *) echo "$DB_TYPE is not a valid value. Only db ( for oltp) and dw (for DW) is allowed"
    printUsage
    exit 1 ;;
esac
...
if [[ $REPLY =~ ^[nN]$ ]]; then
   echo " "
   echo "Please rerun once you have login access"
   exit 1
fi
...
if [[ $REPLY =~ ^[nN]$ ]]; then
   echo " "
   echo "Please rerun with corrected Parameters"
   exit 1
fi
...
if [ -z "$FILE_NAME" ]
then
      echo "There was no DB file found at $ENF_FS_DB_DIR on $ENG_FS_SERVER for $DB_VERSION Payor Version"
      exit 1
else
	 echo "Found file $FILE_NAME at $ENF_FS_DB_DIR on $ENG_FS_SERVER for $DB_VERSION Payor Version"
fi
...
echo "Deleting /tmp/$FILE_NAME as import is complete and freeing up space"
rm -rf /tmp/$FILE_NAME
